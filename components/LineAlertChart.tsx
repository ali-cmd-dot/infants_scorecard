      ...
      */}
      ...
      >
        ...
        {data.map((d, i) => {
          ...
          return (
            <text key={i} x={PAD_L + i * xStep} y={baseY + 16} textAnchor="middle"
        ...
        {SERIES.map(s => {
          ...
          return (
            <path key={`l-${s.key}`} d={smoothPath(pts)} fill="none"
              ...
              strokeOpacity={hoverIdx !== null ? 0.3 : 0.9} />
          );
        })}
                ...
                {active.map((s, i) => {
                  ...
                  return (
                    <g key={`tt-${s.key}`}>
                      ...
                      <text x={bx + 28} y={by + 26 + i * 22} dominantBaseline="middle"
                        style={{ fontFamily: "'Inter',sans-serif", fontSize: "11px", fill: "rgba(255,255,255,0.7)" }}>
                        {s.label}
                      </text>
                      <text x={bx + BOX_W - 10} y={by + 26 + i * 22} textAnchor="end" dominantBaseline="middle"
        ...
        {(() => {
          const ly = VH - 20;
          const lw = 145;
          const startX = (VW - SERIES.length * lw) / 2;
          return (
            <g>
              {SERIES.map((s, i) => {
