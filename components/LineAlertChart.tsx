...
export default function LineAlertChart({ data, title }: Props) {
  // Fixed ViewBox — SVG scales width but height is constant = ZOOM STABLE
  const VW = 900, VH = 460;
  const PAD_L = 64, PAD_R = 28, PAD_T = 24, PAD_B = 72;
  const chartW = VW - PAD_L - PAD_R;
  const chartH = VH - PAD_T - PAD_B;
  const baseY  = PAD_T + chartH;
  // Fixed pixel height for the SVG element itself
  const FIXED_H = 460;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // Scale mouse coords to viewBox coords
    const scaleX = VW / rect.width;
    const mouseX = (e.clientX - rect.left) * scaleX;
    const relX = mouseX - PAD_L;
    setHoverIdx(Math.max(0, Math.min(data.length - 1, Math.round(relX / (xStep || 1)))));
  }, [data.length, xStep, VW]);
  ...
  if (!data.length) {
    return (
      <div>
        {title && <div style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontWeight: 700, fontSize: "15px", color: "#f0f7f0", marginBottom: "16px", letterSpacing: "-0.01em" }}>{title}</div>}
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Area fills — reverse so smaller areas paint on top */}
        ...
        {SERIES.map(s => {
          ...
          if (!pts || pts.length < 2) return null;
          return (
            <path key={`l-${s.key}`} d={smoothPath(pts)} fill="none"
              stroke={s.color} strokeWidth="2"
              strokeOpacity={hoverIdx !== null ? 0.35 : 0.9} />
          );
        })}
        ...
        {hoverX !== null && hoverData && (() => {
          ...
          const BOX_W = 195;
          const BOX_H = 12 + activeSeries.length * 22 + 8;
          ...
          return (
            <>
              ...
              {/* Highlighted line segments on hover */}
              {SERIES.map(s => {
                const pts = pointsMap[s.key];
                if (!pts || hoverIdx === null) return null;
                const pt = pts[hoverIdx];
                if ((hoverData[s.key] as number) === 0) return null;
                return (
                  <path key={`hl-${s.key}`} d={smoothPath(pts)} fill="none"
                    stroke={s.color} strokeWidth="2.5" strokeOpacity="1" />
                );
              })}
              ...
              <g>
                <rect x={bx} y={by} width={BOX_W} height={BOX_H} rx="10"
                  fill="#0e160e" stroke="rgba(74,222,128,0.3)" strokeWidth="1"
                  style={{ filter: "drop-shadow(0 4px 16px rgba(0,0,0,0.7))" }} />
                <text x={bx + 12} y={by + 16}
                  style={{ fontFamily: "'Inter',sans-serif", fontSize: "10px", fill: "rgba(255,255,255,0.45)", fontWeight: 600 }}>
                  {hoverData.date}
                </text>
                {activeSeries.map((s, i) => {
                  ...
                  return (
                    <g key={`tt-${s.key}`}>
                      <circle cx={bx + 16} cy={by + 28 + i * 22} r="4" fill={s.color} />
                      <text x={bx + 28} y={by + 28 + i * 22} dominantBaseline="middle"
                        style={{ fontFamily: "'Inter',sans-serif", fontSize: "11px", fill: "rgba(255,255,255,0.7)" }}>
                        {s.label}
                      </text>
                      <text x={bx + BOX_W - 10} y={by + 28 + i * 22} textAnchor="end" dominantBaseline="middle"
              ...
              {SERIES.map((s, i) => {
                ...
                return (
                  <g key={`leg-${s.key}`}>
                    ...
                    <text x={lx + 26} y={ly} dominantBaseline="middle"
