...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  // ── SUMMARY TAB: Vehicle Number + Score ──
  ...
  // ── ALERTS TAB: plate_number + alarm columns ──
  // Build map: vehicleNumber → cumulative AlertSummary
  ...
  if (alertRows.length > 1) {
    ...
    for (let i = 1; i < alertRows.length; i++) {
      ...
      if (!vehicleAlertMap[plate]) {
        vehicleAlertMap[plate] = emptyAlerts();
      }
      vehicleAlertMap[plate] = addAlerts(vehicleAlertMap[plate], rowAlerts);
    }
  }

  // ── SHEET 2: Vehicle → Client mapping ──
  ...
  // ── COMBINE: Build VehicleData list ──
  ...
  // ── OVERALL ALERTS ──
  const overallAlerts = allVehicles.reduce(
    (acc, v) => addAlerts(acc, v.alerts),
    emptyAlerts()
  );
